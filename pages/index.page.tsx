...
import CardSkeleton from "dh-marvel/components/comics/CardSkeleton";
interface Props {
	comics: Comics;
}
	...
	return (
		<>
			...
			<BodySingle title={"Comics Universo Marvel"}>
				...
				>
					<Pagination
						count={Math.ceil(comics?.data?.total / ITEMS_LIMIT)}
						showFirstButton
						showLastButton
						onChange={handlChangesPage}
						color="primary"
						size="large"
						page={page}
					/>
				...
				<Box sx={{ flexGrow: 2, m: "40px auto 60px" }}>
					<Grid container spacing={{ xs: 2, md: 2 }}>
						{loading
							? Array.from(new Array(ITEMS_LIMIT)).map((_, index) => (
